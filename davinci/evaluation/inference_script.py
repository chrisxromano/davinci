...
It loads an ONNX model, runs inference on GPR scan windows,
and saves binary classification predictions.

Input:  /workspace/input.npy  — (N, 1, depth, width) float32
Output: /workspace/output.npy — (N,) or (N, 1) predictions (labels or probabilities)
...
def main():
    ...
    print(
        f"[INFO] Prediction stats: min={predictions_flat.min():.4f}, "
        f"max={predictions_flat.max():.4f}, mean={predictions_flat.mean():.4f}"
    )
