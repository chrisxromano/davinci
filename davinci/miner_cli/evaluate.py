...
from ..evaluation import calculate_metrics, validate_predictions
from .config import get_test_data
from .errors import (
    EvaluationError,
...
def validate_model_interface(session: ort.InferenceSession) -> str:
    """
    Validate model input/output interface matches expected GPR format.

    Expected input: (batch, 1, depth, width) — single-channel GPR scan windows.
    Expected output: (batch,) or (batch, 1) — binary classification (0/1 or probability).
    ...
    if len(inputs) != 1:
        raise ModelInterfaceError(
            ...
            "Model should have a single input for GPR scan windows."
        )

    input_info = inputs[0]
    input_shape = input_info.shape

    # Expect 4D shape: (batch, channels, depth, width)
    if len(input_shape) != 4:
        raise ModelInterfaceError(
            f"Input shape {input_shape} invalid. "
            "Expected 4D shape (batch, 1, depth, width) for GPR scan windows."
        )

    # Check channel dimension (should be 1 for single-channel GPR)
    channel_dim = input_shape[1]
    if isinstance(channel_dim, int) and channel_dim != 1:
        raise ModelInterfaceError(
            f"Model expects {channel_dim} input channels, but GPR scans are single-channel. "
            "Expected channel dimension of 1."
        )
    ...
    if len(outputs) != 1:
        raise ModelInterfaceError(
            ...
            "Model should output a single classification per sample."
        )
    ...
    if len(output_shape) == 2:
        ...
        if isinstance(out_dim, int) and out_dim != 1:
            raise ModelInterfaceError(
                ...
                "Expected (batch,) or (batch, 1) for binary classification."
            )
...
def run_inference(
    ...
    Args:
        ...
        features: Input array of shape (batch, 1, depth, width).
...
def evaluate_model(
    ...
    4. Calculates classification metrics (accuracy, precision, recall, F1)
    ...
    try:
        binary_preds = validate_predictions(predictions, expected_length=len(ground_truth))
        metrics = calculate_metrics(ground_truth, binary_preds)
        logger.debug(f"F1: {metrics.f1:.2%}, Accuracy: {metrics.accuracy:.2%}")
    except Exception as e:
        return EvaluateResult(
            model_path=str(model_path),
            success=False,
