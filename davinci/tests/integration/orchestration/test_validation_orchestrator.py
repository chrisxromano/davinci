...
from davinci.data import EvaluationDataset, GPRScanWindow
from davinci.incentives import NoValidModelsError
from davinci.orchestration import ValidationOrchestrator
from davinci.tests.fixtures.evaluation.conftest import create_test_model

pytestmark = pytest.mark.integration


@pytest.fixture
def sample_windows() -> list[GPRScanWindow]:
    """Create sample GPR scan windows with binary labels."""
    np.random.seed(42)
    windows = []
    for i in range(20):
...
@pytest.fixture
def validation_dataset(sample_windows: list[GPRScanWindow]) -> EvaluationDataset:
    """Create evaluation dataset from sample GPR windows."""
    return EvaluationDataset(windows=sample_windows)
...
def create_models_with_metadata(
    ...
    Args:
        ...
        depth: Input depth dimension for the models
        width: Input width dimension for the models
    ...
    for i, (seed, block) in enumerate(configs):
        ...
        create_test_model(output_path=model_path, depth=depth, width=width, seed=seed)
...
class TestValidationOrchestratorIntegration:
    ...
    async def test_full_pipeline_single_model(
        self,
        tmp_path: Path,
        validation_dataset: EvaluationDataset,
    ) -> None:
        """Test complete pipeline with a single model."""
        # Create model matching GPR dimensions (depth=32, width=64)
        model_path = tmp_path / "model.onnx"
        create_test_model(output_path=model_path, seed=42)
    ...
    async def test_duplicate_detection_in_pipeline(
        ...
        create_test_model(output_path=original_path, seed=42)

        # Create exact copy
        copy_path = tmp_path / "copy.onnx"
        create_test_model(output_path=copy_path, seed=42)

        # Create unique model
        unique_path = tmp_path / "unique.onnx"
        create_test_model(output_path=unique_path, seed=999)
    ...
    async def test_mixed_success_and_failure(
        ...
        # Create valid model (depth=32, width=64 matches dataset)
        valid_path = tmp_path / "valid.onnx"
        create_test_model(output_path=valid_path, seed=42)

        # Create invalid model (wrong depth dimension)
        invalid_path = tmp_path / "invalid.onnx"
        create_test_model(output_path=invalid_path, depth=99, seed=123)
    ...
    async def test_all_models_fail_raises_error(
        ...
        # Create models with wrong input shape (depth=99 doesn't match dataset depth=32)
        model_paths, chain_metadata = create_models_with_metadata(
            tmp_path, [(0, 1000), (1, 1001), (2, 1002)], depth=99
        )
    ...
    async def test_all_successful_models_are_copiers_raises_error(
        ...
        """
        # Create model that will fail (wrong input shape)
        failing_original = tmp_path / "failing_original.onnx"
        create_test_model(output_path=failing_original, depth=99, seed=42)

        # Create two identical models (one will be marked as copier)
        copy_a = tmp_path / "copy_a.onnx"
        create_test_model(output_path=copy_a, seed=100)

        copy_b = tmp_path / "copy_b.onnx"
        create_test_model(output_path=copy_b, seed=100)  # Same seed = identical
        ...
        create_test_model(output_path=failing_earliest, depth=99, seed=100)
