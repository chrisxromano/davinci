...
from davinci.models import ModelDownloadError, ModelError, SchedulerConfig
from davinci.models.scheduler import ModelDownloadScheduler
...
class TestFilterNeedsDownload:
    ...
    def test_filters_already_cached_models(
        ...
        # First model is cached, second is not
        result = scheduler._filter_needs_download(sample_commitments)
...
class TestDownloadSingle:
    ...
    async def test_returns_success_result_on_successful_download(
        ...
        from davinci.models.downloader import ModelDownloadResult
...
class TestRunPreDownloadTiming:
    ...
    async def test_uses_full_window_when_plenty_of_time(
        ...
        with (
            patch("davinci.models.scheduler.datetime") as mock_datetime,
            patch("davinci.models.scheduler.asyncio.sleep", new_callable=AsyncMock),
        ):
...
class TestCatchUpBlockPreservationE2E:
    ...
    Reproduces the exact bug from production: catch-up resets block to 0,
    which could cause models to lose their verified block number.
    """
    ...
    async def test_old_enough_model_available_after_catch_up(
        ...
        # Download succeeds, real block is 5000
        download_result = ModelDownloadResult(
