...
from davinci.orchestration import ValidationOrchestrator, ValidationResult
...
class TestValidationOrchestratorRun:
    ...
    @pytest.mark.asyncio
    async def test_input_batch_with_channel_dim_passed_to_evaluator(
        self,
        orchestrator: ValidationOrchestrator,
        mock_evaluator: AsyncMock,
        mock_detector: MagicMock,
        ...
        """Input batch with channel dimension (N, 1, D, W) is passed to evaluator."""
        dataset = create_dataset(size=5)
        model_paths = {"A": Path("/model_a.onnx")}
        chain_metadata = {"A": create_chain_metadata("A")}
        ...
        # input_batch is (N, D, W), orchestrator adds channel → (N, 1, D, W)
        assert features.shape == (5, 1, 32, 64)
