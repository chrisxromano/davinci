...
from davinci.chain.models import Metagraph, Neuron
from davinci.models import DownloadResult
from davinci.validator import Validator
...
def mock_config() -> MagicMock:
    ...
    config.eval_data_dir = "/tmp/test_eval_data"
    config.eval_data_poll_seconds = 300
...
def validator(mock_config: MagicMock) -> Validator:
    ...
    with (
        ...
        patch("davinci.validator.validator.bt.subtensor") as mock_subtensor,
        patch("davinci.validator.validator.bt.wallet") as mock_wallet,
        patch("davinci.validator.validator.EvaluationDataLoader"),
        patch("davinci.validator.validator.ValidationOrchestrator"),
    ):
...
class TestGetNextEvalTime:
    ...
    def test_returns_today_if_before_scheduled_time(self, validator: Validator) -> None:
        ...
        with patch("davinci.validator.validator.datetime") as mock_datetime:
...
class TestRunCatchUpIfTime:
    ...
    async def test_extracts_failed_hotkeys_from_download_results(
        ...
        with (
            patch("davinci.validator.validator.datetime") as mock_datetime,
            patch(
                "davinci.validator.validator.asyncio.sleep", new_callable=AsyncMock
            ),
        ):
            mock_datetime.now.return_value = mock_now
            mock_datetime.side_effect = lambda *args, **kw: datetime(*args, **kw)
...
class TestPreDownloadLoop:
    ...
    async def test_runs_immediately_when_past_pre_download_start(
        ...
        with (
            patch("davinci.validator.validator.datetime") as mock_datetime,
            patch("davinci.validator.validator.asyncio.sleep", mock_sleep),
        ):
            mock_datetime.now.return_value = mock_now
            mock_datetime.side_effect = lambda *args, **kw: datetime(*args, **kw)
