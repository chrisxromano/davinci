...
def add_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        dest="eval_data_dir",
        type=str,
        help="Directory where the subnet owner drops evaluation .npz files.",
        default=os.environ.get("EVAL_DATA_DIR", "./eval_data"),
    )

    parser.add_argument(
        "--eval_data.poll_seconds",
        dest="eval_data_poll_seconds",
        type=int,
        help="Seconds between polls for new evaluation data files.",
        default=int(os.environ.get("EVAL_DATA_POLL_SECONDS", "300")),
    )

    parser.add_argument(
        "--eval_data.schedule_hour",
        dest="validation_data_schedule_hour",
        type=int,
        help="Hour (UTC) for scheduled daily evaluation (0-23). Used for pre-download timing.",
        default=int(os.environ.get("EVAL_DATA_SCHEDULE_HOUR", "18")),
    )

    parser.add_argument(
        "--eval_data.schedule_minute",
        dest="validation_data_schedule_minute",
        type=int,
        help="Minute for scheduled daily evaluation (0-59). Used for pre-download timing.",
        default=int(os.environ.get("EVAL_DATA_SCHEDULE_MINUTE", "0")),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable logging per-window predictions table to WandB (disabled by default).",
...
def get_config() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="DaVinci Subnet Validator",
    ...
    config.eval_data_dir = Path(config.eval_data_dir)
...
def config_to_dict(config: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "eval_data_dir": str(config.eval_data_dir),
        "eval_data_poll_seconds": config.eval_data_poll_seconds,
...
def setup_logging(level: str) -> None:
    ...
    - DEBUG: Only davinci.* debug logs, third-party at WARNING
