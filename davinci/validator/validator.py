...
from davinci.chain.errors import ChainConnectionError
from davinci.chain.models import Metagraph

if TYPE_CHECKING:
    from davinci.chain import ChainClient
...
from davinci.models import (
    DownloadConfig,
    DownloadResult,
    SchedulerConfig,
    create_model_scheduler,
)
from davinci.observability import WandbLogger, create_wandb_logger
from davinci.orchestration import ValidationOrchestrator
from davinci.utils.misc import ttl_get_block
...
class Validator:
    ...
    def __init__(self, config: argparse.Namespace):
        ...
        from davinci.chain import PylonConfig
        ...
        # Evaluation data loader — reads .npz files dropped by subnet owner
        ...
        self.validation_data: EvaluationDataset | None = None
    ...
    def _apply_burn(self, weights: dict[str, float]) -> dict[str, float]:
        ...
        """
        burn_amount: float = 0.5  # Hardcoded so autoupdater enforces consistent burn across validators
    ...
    def _try_load_eval_data(self) -> bool:
        """
        Try to load evaluation data from the data directory.

        Returns:
            True if new data was loaded, False if no data available.
        """
        try:
        ...
        if len(dataset) == 0:
            logger.warning("Evaluation data is empty, skipping")
            return False

        self.validation_data = dataset
        logger.info(f"Evaluation data loaded: {dataset}")
        return True

    async def _run_evaluation(self, dataset: EvaluationDataset) -> None:
        """
